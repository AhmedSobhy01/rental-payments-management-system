...
window.disableBtn = function (btn, loader) {
  var removedClasses = [];
  if (btn.dataset.hasOwnProperty("removedClasses")) removedClasses.push.apply(removedClasses, _toConsumableArray(btn.dataset.removedClasses.split(",")));
  ...
  if (loader === true) {
    btn.innerHTML = '<i class="fas fa-spinner animate-spin mr-2"></i>' + btn.innerHTML;
  }
};

window.enableBtn = function (btn) {
  btn.classList.remove("cursor-not-allowed", "bg-opacity-50");
  btn.dataset.removedClasses.split(",").forEach(function (cls) {
    return cls ? btn.classList.add(cls) : false;
  });
  delete btn.dataset.removedClasses;
  btn.disabled = false;
  btn.innerHTML = btn.innerHTML.replace('<i class="fas fa-spinner animate-spin mr-2"></i>', "");
};
...
document.querySelectorAll(".submit-disabled").forEach(function (elm) {
  return elm.addEventListener("submit", function (evt) {
    return disableBtn(evt.target.querySelector("button[type=submit]"), true);
  });
});
})();
