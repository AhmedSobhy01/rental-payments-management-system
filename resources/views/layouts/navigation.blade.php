<header class="relative bg-white dark:bg-darker">
    <div class="flex items-center {{ pageTabActive('admin.') ? 'justify-between' : 'justify-end' }} md:justify-end p-2 border-b dark:border-primary-darker">
        ...
        <div class="flex justify-between items-center">
            ...
            <nav x-data="{ langsOpen: false }" class="relative mr-5">
                ...
                <div x-show="langsOpen" @click="langsOpen = false"
                ...
                <nav class="space-x-2 flex items-center mr-3 z-50">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open; $nextTick(() => { if(open){ $refs.userMenu.focus() } })" type="button" aria-haspopup="true" :aria-expanded="open ? 'true' : 'false'" class="transition-opacity duration-200 rounded-full dark:opacity-75 dark:hover:opacity-100 focus:outline-none focus:ring dark:focus:opacity-100">
                            ...
                            <img class="w-10 h-10 rounded-full" src="{{ 'https://ui-avatars.com/api/?background=0D8ABC&color=fff&name=' . urlencode(auth()->user()->name) }}" alt="Ahmed Kamel" />
                        </button>
                        <!-- User dropdown menu -->
                        <div
                            x-show="open"
                        ...
                        >
                            @if (!pageTabActive('admin'))
                            <a href="{{ route('admin.dashboard') }}" role="menuitem" class="block px-4 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:text-light dark:hover:bg-primary">
                                {{ __('app.Admin Dashboard') }}
                            </a>
                            @endif

                            <a href="{{ route('admin.settings.account') }}" role="menuitem" class="block px-4 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:text-light dark:hover:bg-primary">
                                {{ __('app.Edit Account') }}
                            </a>
                            <form method="POST" action="{{ route('logout') }}" role="menuitem" class="block px-4 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:text-light dark:hover:bg-primary">
                                @csrf
                                <button type="submit" class="w-full {{ LaravelLocalization::getCurrentLocaleDirection() == 'ltr' ? 'text-left' : 'text-right' }}">{{ __('app.Logout') }}</button>
                            </form>
                        </div>
                    </div>
                </nav>
            @else
                <a href="{{ route('login') }}" class="bg-blue-500 text-white px-6 py-2 rounded font-medium mr-3 hover:bg-blue-600 transition duration-200 each-in-out">{{ __('app.Login') }}</a>
            @endauth
        </div>
    </div>
